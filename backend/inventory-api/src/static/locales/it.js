window.itMessages = {
    notifications: {
        itemAdded: 'Articolo aggiunto correttamente',
        itemUpdated: 'Articolo aggiornato correttamente',
        itemDeleted: 'Articolo eliminato correttamente',
        error: 'Operazione non riuscita. Riprova più tardi',
        insufficientStock: 'Scorte insufficienti: {itemName}',
        itemUsedSuccessfully: 'Utilizzato 1 {unit} di {itemName} (rimanenti: {quantity} {unit})'
    },
    app: {
        title: 'Gestione Inventario Domestico Intelligente'
    },
    nav: {
        inventory: 'Inventario',
        add: 'Aggiungi articolo',
        purchaseList: 'Lista acquisti',
        menu: 'Menu',
        language: 'Lingua'
    },
    purchaseList: {
        description: 'Articoli da rifornire in base ai livelli di scorta attuali',
        totalItems: 'Articoli da acquistare',
        currentQuantity: 'Quantità attuale',
        minQuantity: 'Scorta minima',
        suggestedQuantity: 'Quantità suggerita',
        unit: 'Unità',
        lastUsed: 'Ultimo utilizzo',
        refresh: 'Aggiorna lista',
        print: 'Stampa lista'
    },
    inventory: {
        title: 'Inventario',
        search: 'Cerca articoli...',
        allCategories: 'Tutte le categorie',
        name: 'Nome articolo',
        category: 'Categoria',
        quantity: 'Quantità',
        unit: 'Unità',
        expiryDate: 'Data di scadenza',
        status: 'Stato',
        actions: 'Azioni',
        showExpired: 'Mostra solo gli articoli scaduti'
    },
    categories: {
        food: 'Alimentari',
        medicine: 'Medicinali',
        cleaning: 'Prodotti per la pulizia',
        personal: 'Cura della persona',
        household: 'Casalinghi',
        electronics: 'Elettronica'
    },
    stats: {
        totalItems: 'Articoli totali',
        lowStock: 'Scorte basse',
        expiringSoon: 'In scadenza',
        categories: 'Categorie'
    },
    add: {
        title: 'Aggiungi nuovo articolo',
        editTitle: 'Modifica articolo',
        name: 'Nome articolo',
        category: 'Categoria',
        quantity: 'Quantità',
        unit: 'Unità',
        minQuantity: 'Scorta minima',
        expiryDate: 'Data di scadenza',
        description: 'Descrizione',
        selectCategory: 'Seleziona categoria',
        cancel: 'Annulla',
        save: 'Salva',
        update: 'Aggiorna'
    },
    status: {
        normal: 'Normale',
        lowStock: 'Scorte basse',
        expiringSoon: 'In scadenza',
        expired: 'Scaduto'
    }
};